"""
Sidharrth Nagappan
Improved Multi-Objective Asynchronous Successive Halving Scheduler (MO-ASHA)
(some extra ranking)

Still issues, keeping for future work
...
class MultiObjectiveAsyncHyperBandScheduler(FIFOScheduler):
    ...
    def on_trial_result(self, tune_controller, trial, result):
        ...
        action = bracket.on_result(trial, resource, metrics, reduction_factor=self.reduction_factor)
        return action
    ...
    def on_trial_complete(self, tune_controller, trial, result):
        ...
        if bracket:
            ...
            metrics = np.array([result[obj] for obj in self.objectives]) * self.sign_vector
            bracket.on_result(trial, resource, metrics, reduction_factor=self.reduction_factor)
...
class MultiObjectiveBracket:
    ...
    def on_result(self, trial, resource, metrics, reduction_factor):
        """
        ...
        for rung in self.rungs:
            ...
            print(f"Recorded in rung: {recorded}")
            ...
            print(f"Re-ordered existing solutions: {candidates}")
            
            # check if new trial dominated by rung's best solutions
            if candidates is not None and not self._is_promotable(metrics, candidates, reduction_factor=reduction_factor):
    ...
    def _calculate_candidates(self, recorded):
        ...
        print(f"Recorded points (outer points): {points}")
        print(f"Non-dominated points (outer function): {nd_points}")
    ...
    def _is_promotable(self, new_metrics, candidates, reduction_factor):
        """
        ...
        # add the new_metrics to candidates
        # redo either eps_net or nsga_ii
        # check if this new trial still makes the cut
        print(f"New metrics: {new_metrics}")
        print(f"Candidates before adding metrics: {candidates}")

        if len(candidates) == 0:
            new_candidates = np.array([new_metrics])
        else:
            new_candidates = np.vstack([candidates, new_metrics])
            
        print(f"Candidates after adding metrics: {new_candidates}")    
        
        if self.strategy == MOSelectionStrategies.EPS_NET:
            new_candidates = self._eps_net(new_candidates)
        elif self.strategy == MOSelectionStrategies.NSGA_II:
            new_candidates = self._nsga_ii(new_candidates)
                        
        if len(new_candidates) > 1 and new_metrics in new_candidates[:max(1, math.ceil(1 / (1 - reduction_factor)))]:
            print(f"Promotable: {new_metrics in new_candidates[:max(1, math.ceil(1 / (1 - reduction_factor)))]}")
            return True
        else:
            print(f"Not enough to promote: {new_metrics} not in top {max(1, math.ceil(1 / (1 - reduction_factor)))}")
            return False
        
        # if top_selection_idx < len(new_candidates):
        #     print(f"Not enough to promote: {top_selection_idx} < {len(new_candidates)}")
        #     print(f"Promoting anyway")
        #     return True
        # else:
        #     # if new_metrics is in top x% of new_candidates, then it's promotable
        #     print(f"Promotable: {new_metrics in new_candidates[:top_selection_idx]}")
        #     return new_metrics in new_candidates[:top_selection_idx]
