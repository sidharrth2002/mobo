...
class AxSearchMultiObjective(Searcher):
    ...
    def __init__(
        ...
        **ax_kwargs
        ...
        if metric and mode and len(metric) != len(mode):
            raise ValueError(
                "Number of metrics and modes must be the same. Got "
                "{} metrics and {} modes.".format(len(metric), len(mode))
            )

        self._metrics = metric
        self._modes = mode

        super(AxSearchMultiObjective, self).__init__(
            metric=metric,
            mode=mode,
        )
    ...
    def _setup_experiment(self):
        ...
        if not has_experiment:
            ...
            objectives = {}
            for metric, mode in zip(self._metrics, self._modes):
                minimize = mode == "min"
                objectives[metric] = ObjectiveProperties(
                    minimize=minimize, threshold=0.5
                )
    ...
    def set_search_properties(
        ...
        if mode:
            self._modes = mode if isinstance(mode, list) else [mode]
    ...
    def suggest(self, trial_id: str) -> Optional[Dict]:
        ...
        if not self._metrics or not self._modes:
            raise RuntimeError(
                UNDEFINED_METRIC_MODE.format(
                    cls=self.__class__.__name__, metric=self._metrics, mode=self._modes
                )
            )
